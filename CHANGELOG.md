...
- `make qemu-test-full-doom-taxonomy` PASS with `visual_gameplay=PASS`, QEMU audio on, and the DOOM visual gate launched as `run DOOM.EXE -nosfx` using music-enabled audio config on the ALSA backend.
...
- `DRVLOAD_ARGS=/AUDIO QEMU_TIMEOUT_SEC=260 bash scripts/qemu_test_full_drvload_smoke.sh` PASS with QEMU audio on (`backend=pipewire`, SB16 `0x220`, IRQ 7), including `DSP OK at 0x0220`, `TONE DONE`, and `OK AUDIO` markers.
