...
## Unreleased
1. Repository reset avviato: nuova linea architetturale legacy BIOS x86 (no UEFI nel nuovo core).
2. Archivio completo del progetto precedente in `OLD/archive-2026-04-22/`.
3. Nuova base documentale v2: architettura, roadmap, diario di bordo, direttive operative agenti AI.
4. Nuova struttura build profile iniziale (`floppy` e `full`) con scaffolding artefatti.
...
2. Added the IDT gate encoder for the v8086 monitor: `vm86_idt_encode_gate`, `vm86_idt_encode`, `vm86_idt_read_offset`, `vm86_idt_read_selector`, `vm86_idt_read_type`. Writes all 256 8-byte 32-bit interrupt gates (type 0x8E) per Intel SDM Vol.3A §6.11: [offset_lo, selector, 0, type_attr, offset_hi]. A spurious trampoline fills every vector; 10 vectors in the `VM86_IDT_VEC_*` enum (DE, UD, NM, TS, NP, SS, GP, PF, SW20, SW21) are overwritten with dedicated handlers. #GP (0x0D) is the primary v8086 trap per design §5.2. IDTR limit pre-computed as `VM86_IDT_BYTES-1` and emitted as an audit marker. No `LIDT` load performed.
3. Added the v8086 IRET stack-frame encoder: `vm86_iret_encode_frame` plus 9 read-back helpers. Writes the 36-byte, 9-dword frame in the exact SDM Vol.3A §20.2.1 layout (EIP@+0, CS@+4, EFLAGS@+8, ESP@+12, SS@+16, ES@+20, DS@+24, FS@+28, GS@+32). Safety-critical invariant: `VM86_EFLAGS_VM | VM86_EFLAGS_IOPL3 | VM86_EFLAGS_RESERVED1` are OR'd into EFLAGS unconditionally, so a caller passing `eflags=0` still produces a frame the CPU will consume as a v8086 entry rather than falling back into ring-0 PE32. No IRET executed.
4. Added two static gates: `scripts/test_vm86_gdt_encode.sh` (59 OK / 0 FAIL) and `scripts/test_vm86_idt_iret.sh` (77 OK / 0 FAIL). Both audit sentinel, prototypes, every byte-layout write, marker ordering (11 markers for GDT, 13 for IDT+IRET), per-field probe assertions, and enforce no-live-call and no-inline-asm `lgdt`/`lidt`/`iret` invariants. Wired into `Makefile` as `test-vm86-gdt-encode` and `test-vm86-idt-iret`.
5. The v86 monitor regression stack is now 10 gates wide (017..026), all PASS post-merge; encoders are observability-only with no invocation from `shell.c` / `stage2.c`. Phases 027+ (assembly trampoline, live LGDT/LIDT, first IRET into v8086, gem.exe wire-up) are deferred pending explicit user authorization since they would break the single-session observability-only discipline.
6. Bumped baseline to `CiukiOS Alpha v0.8.9`.

## v0.8.8
1. Added the INT 21h AH=4Ch DOS clean-exit handler (OPENGEM-021): `vm86_int21_4c_handler` performs the exit transition `task.state := VM86_TASK_STATE_EXITED`, `task.exit_reason := VM86_EXIT_REASON_INT21_4C`, `task.exit_errorlevel := AL` (byte width from `frame->eax & 0xFF`), with null-safety on both `task` and `frame`. Observability-only through `vm86_int21_4c_probe`; registered in the dispatcher but never invoked from the live boot path.
2. Added the host-side console sink for v8086 guest writes (OPENGEM-022): `vm86_console_sink` (256-byte ring buffer + overflow counter + attach/detach/reset API). `vm86_console_sink_put_byte` absorbs guest output for gate auditing independent of the serial stream.
3. Added the DOS/INT 20h console handler set (OPENGEM-022): `vm86_int20_handler` (program terminate → exit reason `INT20_TERMINATE`, errorlevel 0); `vm86_int21_02_handler` (character output from DL); `vm86_int21_09_handler` (DOS `$`-terminated string from DS:DX), with guest linear-address computation `(DS << 4) + (DX & 0xFFFF)` honouring the conventional-memory window in the task descriptor.
4. Added the BIOS INT 10h AH=0Eh teletype handler (OPENGEM-023): `vm86_int10_0e_handler` emits AL into the console sink, isolating BIOS-vs-DOS output paths while keeping a single byte stream auditable by gate probes.
5. Added the INT 21h AH=30h DOS-version query handler (OPENGEM-024): `vm86_int21_30_handler` preserves the upper halves of EAX/EBX, writes `AL=VM86_DOS_VERSION_MAJOR=0x05`, `AH=VM86_DOS_VERSION_MINOR=0x00`, `BH=0xFF` (generic OEM), `BL=0`, and `CX=0`; increments `task.int_count`. Matches the reference environment used to validate the symbiotic FreeDOS pipeline.
6. Added the `gem.exe` T0 readiness simulation (OPENGEM-024): `vm86_gem_t0_readiness_probe` seeds MZ-like entry coordinates (CS=0x1000, IP=0x0100, SS=0x1000, SP=0xFFFE) + a 256-byte synthetic conventional window with a `GEM$` banner, registers the full T0 handler set, rotates the single INT 21h slot across AH=30h → AH=09h → AH=4Ch across four dispatches interleaved with one INT 10h AH=0Eh teletype, and cross-checks 17 post-conditions (dispatch statuses, register byte lanes, sink content, task state, exit reason, errorlevel, int_count) while emitting 11 ordered markers ending in an explicit `ready-surface` / `pending-surface` manifest.
7. Added five new static gates driven by bit-level probe assertions: `test-vm86-int21-4c` (26 OK), `test-vm86-console` (47 OK), `test-vm86-int10-0e` (26 OK), `test-vm86-gem-t0` (47 OK), plus the carry-over `test-vm86-dispatcher` from the 020 landing; v86 regression stack grew to 8 PASS.
8. Bumped baseline to `CiukiOS Alpha v0.8.8`.
