...
4. Added SB16 probe/playback evidence through `SB16INIT.COM`, `DRVLOAD.COM /AUDIO`, QEMU SB16 device wiring for explicit full-profile audio runs plus an audio-aware DRVLOAD smoke path. Game and taxonomy lanes now keep QEMU audio off by default so DOOM/WOLF3D do not auto-detect the partial SB16 path; the validated explicit path detects the DSP at `0x220` and completes a direct-DAC tone.
