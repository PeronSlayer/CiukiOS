...
COM_CIUKDEMO_SRC := com/ciukdemo/ciukdemo.c
COM_CIUKDEMO_ELF := build/CIUKDEMO.COM.elf
COM_CIUKDEMO_BIN := build/CIUKDEMO.COM
COM_GFXDOOM_SRC := com/gfxdoom/gfxdoom.c
COM_GFXDOOM_ELF := build/GFXDOOM.COM.elf
COM_GFXDOOM_BIN := build/GFXDOOM.COM
COM_WADVIEW_SRC := com/wadview/wadview.c
COM_WADVIEW_ELF := build/WADVIEW.COM.elf
COM_WADVIEW_BIN := build/WADVIEW.COM
...
COM_M6_DPMI_FREE_SMOKE_SRC := com/m6_dpmi_free_smoke/ciukrel.c
COM_M6_DPMI_FREE_SMOKE_ELF := build/CIUKREL.EXE.elf
...
$(COM_CIUKDEMO_BIN): $(COM_CIUKDEMO_SRC) com/ciukdemo/linker.ld boot/proto/services.h | build
	@mkdir -p build/obj/com
	$(CC) $(COM_CFLAGS) -c $(COM_CIUKDEMO_SRC) -o build/obj/com/ciukdemo.o
	$(LD) -nostdlib -z max-page-size=0x1000 -T com/ciukdemo/linker.ld -o $(COM_CIUKDEMO_ELF) build/obj/com/ciukdemo.o
	llvm-objcopy --set-section-flags .data=alloc,load,contents,data -O binary $(COM_CIUKDEMO_ELF) $(COM_CIUKDEMO_BIN)
...
$(COM_WADVIEW_BIN): $(COM_WADVIEW_SRC) com/wadview/linker.ld boot/proto/services.h | build
	@mkdir -p build/obj/com
	$(CC) $(COM_CFLAGS) -c $(COM_WADVIEW_SRC) -o build/obj/com/wadview.o
	$(LD) -nostdlib -z max-page-size=0x1000 -T com/wadview/linker.ld -o $(COM_WADVIEW_ELF) build/obj/com/wadview.o
	llvm-objcopy --set-section-flags .data=alloc,load,contents,data -O binary $(COM_WADVIEW_ELF) $(COM_WADVIEW_BIN)
...
$(COM_M6_DPMI_FREE_SMOKE_PAYLOAD): $(COM_M6_DPMI_FREE_SMOKE_SRC) com/m6_dpmi_free_smoke/linker.ld boot/proto/services.h | build
	@mkdir -p build/obj/com
	$(CC) $(COM_M6_DPMI_FREE_SMOKE_CFLAGS) -c $(COM_M6_DPMI_FREE_SMOKE_SRC) -o build/obj/com/ciukrel.o
	$(LD) -nostdlib -z max-page-size=0x1000 -T com/m6_dpmi_free_smoke/linker.ld -o $(COM_M6_DPMI_FREE_SMOKE_ELF) build/obj/com/ciukrel.o
	llvm-objcopy -O binary $(COM_M6_DPMI_FREE_SMOKE_ELF) $(COM_M6_DPMI_FREE_SMOKE_PAYLOAD)
...
test-vm86-gem-t0:
	bash ./scripts/test_vm86_gem_t0.sh

test-vm86-gdt-encode:
	bash ./scripts/test_vm86_gdt_encode.sh
...
test-vm86-live-plan:
	bash ./scripts/test_vm86_live_plan.sh
...
test-vm86-compat-task:
	bash ./scripts/test_vm86_compat_task.sh
...
test-legacy-v86-live:
	bash ./scripts/test_legacy_v86_live.sh

test-mstest-shell:
	bash ./scripts/test_mstest_shell.sh
...
test-ciukdemo-smoke:
	bash ./scripts/test_ciukdemo_smoke.sh
