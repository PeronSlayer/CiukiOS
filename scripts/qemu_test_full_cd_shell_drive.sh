...
PREFIX="build/full/qemu-full-cd-shell-drive"
...
MON_SOCK="/tmp/ciukios-full-cd-shell-drive.monitor.sock"
...
Artifacts:
  build/full/qemu-full-cd-shell-drive.{serial.log,strings.log,stderr.log,commands.log,meta}
TXT
}
...
mark_fail() {
  local marker="$1"
  local detail="$2"
  echo "[full-cd-shell-drive] FAIL ${marker}: ${detail}" >&2
...
mark_pass() {
  local marker="$1"
  echo "[full-cd-shell-drive] PASS ${marker}"
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "[full-cd-shell-drive] ERROR: unknown option: $1" >&2
      usage
      exit 1
      ;;
  esac
...
if ! wait_for_regex_from_offset "$SERIAL_LOG" "$APPS_PROMPT_PATTERN" 0 "$PROMPT_TIMEOUT_SEC"; then
  mark_fail "INITIAL_D_APPS_PROMPT" "initial D:\\APPS shell prompt not detected"
fi
...
send_and_wait_for_prompt 'cd C:APPS' "$APPS_PROMPT_PATTERN" "CD_C_REL_DEFAULT_D_APPS" "$COMMAND_TIMEOUT_SEC"
send_and_wait_for_prompt 'cd D:/APPS/..' "$ROOT_PROMPT_PATTERN" "CD_D_PARENT_ROOT" "$COMMAND_TIMEOUT_SEC"
send_and_wait_for_prompt 'cd D:/APPS' "$APPS_PROMPT_PATTERN" "CD_D_ABS_APPS" "$COMMAND_TIMEOUT_SEC"
send_and_wait_for_pattern_and_prompt 'cd Z:\' "$CD_ERROR_PATTERN" "$APPS_PROMPT_PATTERN" "CD_BAD_DRIVE_REJECT" "$COMMAND_TIMEOUT_SEC"
...
echo "[full-cd-shell-drive] PASS (full-CD shell drive sequence verified)"
