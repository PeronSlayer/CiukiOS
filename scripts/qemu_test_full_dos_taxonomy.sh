...
DOS_TAXONOMY_PRE_LAUNCH_COMMAND="${DOS_TAXONOMY_PRE_LAUNCH_COMMAND:-}"
...
classify_runtime() {
  ...
  if [[ "$DOS_TAXONOMY_LAUNCH" == "1" ]]; then
    ...
    if ! wait_for_socket "$QEMU_MON_SOCK" 20; then
      ...
      if [[ -n "$DOS_TAXONOMY_PRE_LAUNCH_COMMAND" ]]; then
        if send_text_and_enter "$QEMU_MON_SOCK" "$QEMU_CMD_LOG" "$DOS_TAXONOMY_PRE_LAUNCH_COMMAND"; then
          wait_for_shell_prompt "$LOG_FILE" 30 || true
        fi
      fi
